...
Este archivo descargado, contiene un script un poco largo:
...
Al inicio del código, podemos ver fácilmente la bandera segmentada en 3 diferentes variables, la segunda variable dinámica es la que contiene una parte perdida de la bandera.

Luego, mas abajo vemos como tiene una función de de entrada de licencia en la cual solicita una llave para hacer verificación, dicha llave la verifica con una variable que se encuentra en las primeras lineas del código: `bUsername_trial = b"GOUGH`. SI ingresamos la llave correcta iniciara un código de descifrado.

Luego, comenzara a verificar partes de caracteres para formar de manera dinámica, la parte faltan te de la bandera.

Vemos que cada carácter que conforman la parte dinámica de la bandera, se ejecutan según el carácter, entonces ejecutamos cada una de estas instrucciones y observamos la salida de cada uno:
...
>>> import hashlib
>>> hashlib.sha256(b"GOUGH").hexdigest()
'e8a1f9146d32473b9605568ca66f7b5c2db9f271f57a8c8e9e121e48accddf2f'
>>> hashlib.sha256(b"GOUGH").hexdigest()[4]
'f'
>>> hashlib.sha256(b"GOUGH").hexdigest()[5]
'9'
>>> hashlib.sha256(b"GOUGH").hexdigest()[3]
'1'
>>> hashlib.sha256(b"GOUGH").hexdigest()[6]
'1'
>>> hashlib.sha256(b"GOUGH").hexdigest()[2]
'a'
>>> hashlib.sha256(b"GOUGH").hexdigest()[7]
'4'
>>> hashlib.sha256(b"GOUGH").hexdigest()[1]
'8'
>>> hashlib.sha256(b"GOUGH").hexdigest()[8]
'6'
>>> 
```

Las 3 partes que conformaban la bandera son:
```
key_part_static1_trial = "picoCTF{1n_7h3_|<3y_of_"
key_part_dynamic1_trial = "xxxxxxxx"
key_part_static2_trial = "}"
...
Formamos la variable dinámica:
`f911a486`

Y luego la bandera completa:
...
- https://www.youtube.com/watch?v=**SOyapbV5zEg