...
Encontré este cifrado en un libro antiguo. ¿Puedes entender lo que dice? Conéctese con `nc jupiter.challenges.picoctf.org 58295.`
1. Existen herramientas que lo hacen fácil.
2. Quizás mirar la historia ayude
## Solución

Si hacemos la conexion a donde se indida, obtenemos el siguiente mensaje:
```
...
Sabemos de antemano, que se trata de el cifrado de Vigenère (por lo que nos dice las pistas), asi que vamos a buscar en la web una herramienta que nos ayude a descifrar el cifrado con ayuda de análisis de frecuencias. Obtenemos el siguiente resultado:
```
...
```
Aquí encontramos la bandera.

## Notas adicionales

- `Analisis de frecuencias en criptografia`-> En el criptoanálisis, la técnica de análisis de frecuencias consiste en el aprovechamiento de estudios sobre la frecuencia de las letras o grupos de letras en los idiomas para poder establecer hipótesis para aprovecharlas para poder descifrar un texto cifrado sin tener la clave de descifrado (romper).
## Referencias
